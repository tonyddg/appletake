...
def sample_param(trial: Trial):
    params = {
        ...
        # "total_timesteps": trial.suggest_categorical("total_timesteps", [int(2e4), int(5e4), int(1e5)])
    }
...
if __name__ == "__main__":
    with SafePyRep("scene/socket_plug/socket_plug_vec4_tol5.ttt", True) as pr:
        ...
        em = ExpManager(
            load_exp_config(
                "app/socket_plug/conf/normal_ext_env.yaml",
                "app/socket_plug/finetuning/ext_sac_raw.yaml"
            ),
            sample_param,
