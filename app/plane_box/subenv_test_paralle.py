...
if __name__ == "__main__":
    ...
    with SafePyRep("scene/plane_box/paralle.ttt") as pr:
        env = ParalleSubEnv(
            name_suffix = "", 
            obs_trans = exp_replace_arg_dict()["paralle_ext_trans"],
            obs_source = "depth",
            env_init_box_pos_range = (
                np.array([-40, -40, 0, -5, -5, -10], dtype = np.float32),
                np.array([40, 40, 50, 5, 5, 10], dtype = np.float32)
            ),
            ...
            # 原始尺寸 0.15 x 0.15 x 0.15
            paralle_fixbox_size_offset_range = (
                np.array([-0.03, -0.03, 0], dtype = np.float32),
                np.array([0.15, 0.15, 0.05], dtype = np.float32)
            )
        )
