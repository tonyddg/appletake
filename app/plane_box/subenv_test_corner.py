...
if __name__ == "__main__":
    ...
    with SafePyRep("scene/plane_box/corner.ttt") as pr:
        env = CornerSubEnv(
            name_suffix = "", 
            obs_trans = exp_replace_arg_dict()["corner_ext_trans"],
            obs_source = "depth",
            env_init_box_pos_range = (
                np.array([-25, -25, 15, -2, -2, -5], dtype = np.float32),
                np.array([25, 25, 35, 2, 2, 5], dtype = np.float32)
            ),
            ...
            corner_plane_height_offset_range = (-0.03, 0.03)
        )
