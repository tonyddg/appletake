...
from sample_param import sample_param_ext_sac

if __name__ == "__main__":
    with SafePyRep("scene/plane_box/paralle_vec4_test2.ttt", True) as pr:

        em = ExpManager(
            load_exp_config(
                "app/plane_box/conf/base_hard_env.yaml",
                "app/plane_box/conf/paralle_hard_vec_env.yaml",
                "app/plane_box/finetuning/vec_sac_raw.yaml",
                merge_dict = {
                    "trial": {"meta_exp_root": "runs/plane_box/paralle_finetuning/vec_sac_hard_fine"}
                },
                is_resolve = False
            ),
