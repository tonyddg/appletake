...
SelfHybridOnPolicyAlgorithm = TypeVar("SelfHybridOnPolicyAlgorithm", bound="HybridOnPolicyAlgorithm")

from .hybrid_policy import HybridActorCriticPolicy
from .hybrid_rollout import HybridRolloutBuffer
from .utility import concat_numpy_action_for_step
...
class HybridOnPolicyAlgorithm(BaseAlgorithm):
    """
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                (continue_actions, discrete_actions), values, (continue_log_prob, discrete_log_prob) = self.policy(obs_tensor)
            # actions = actions.cpu().numpy()
            continue_actions = continue_actions.cpu().numpy()
            discrete_actions = discrete_actions.cpu().numpy()

            # Rescale and perform action
            # clipped_actions = actions
            clipped_continue_actions = continue_actions
            ...
            if isinstance(self.action_space, spaces.Box):
                if self.policy.squash_output:
                    ...
                    clipped_continue_actions = self.policy.unscale_action(clipped_continue_actions)
                else:
                    # Otherwise, clip the actions to avoid out of bound error
                    # as we are sampling from an unbounded Gaussian distribution
                    # clipped_actions = np.clip(actions, self.action_space.low, self.action_space.high)
                    clipped_continue_actions = np.clip(clipped_continue_actions, self.action_space.low, self.action_space.high)

            # new_obs, rewards, dones, infos = env.step((clipped_actions))
            new_obs, rewards, dones, infos = env.step(concat_numpy_action_for_step(discrete_actions, clipped_continue_actions))
    ...
    def learn(
        self: SelfHybridOnPolicyAlgorithm,
        total_timesteps: int,
