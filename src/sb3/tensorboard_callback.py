...
class TensorboardEvalCallback(BaseCallback):
    ...
    def _eval_model(self):
        ...
        episode_return_list, episode_length_list = eval_record_to_tensorbard(
            model = self.model,
    ...
    def _need_eval(self) -> bool:
        ...
        return self.num_timesteps >= eval_step or is_last_eval

    def _on_step(self) -> bool:
        # 每个间隔或到达最后一步时进行测试
        if self.num_timesteps % self.eval_freq == 0 or self.num_timesteps == self.locals["total_timesteps"]:
            self._eval_model()
...
class OptunaEvalCallback(TensorboardEvalCallback):
    def __init__(
        ...
        **eval_kwargs
        ...
        super().__init__(tb_writer, eval_freq, n_eval_episodes, deterministic, return_gamma, eval_env, tb_eval_tag_prefix, tb_eval_summary_root, tb_record_flag, save_model, save_data, save_root, num_record_episodes, fps, verbose, **eval_kwargs)
