...
def rot_to_rotvec(q: np.ndarray, is_deg: bool = True) -> Tuple[float, np.ndarray]:
    '''
    将 quat(4)/eular(3, abc 欧拉角) 转为轴角对, 返回值 (rot, vec), 单位 deg
    is_deg 返回值的类型, 如果传入欧拉角, 同时也表示欧拉角的类型
    '''
    if q.shape[0] == 4:
        rotvec = R.from_quat(q).as_rotvec(is_deg)
    elif q.shape[0] == 3:
        rotvec = R.from_euler("zyx", np.flip(q), is_deg).as_rotvec(is_deg)
    else:
        raise Exception("无效的角度表示")
...
class DistSampler:
    def __init__(
        self,
        # float (-1, 1) 截断正态分布的 3 sigma 范围; None 均匀分布
        three_sigma_bound: Optional[float] = None
    ):
        if three_sigma_bound is None:
            self.dist = None
        else:
            sigma = three_sigma_bound / 3
            self.dist = truncated_normal(0, sigma, -1, 1)
        pass
    ...
    def sample_vec(self, min_vec: np.ndarray, max_vec: np.ndarray):
        ...
        if self.dist is None:
            ...
            rnd_base = np.asarray(self.dist.rvs(max_vec.shape), np.float32)
    ...
    def sample_float(self, min_side: float, max_side: float):
        ...
        if self.dist is None:
            ...
            rnd_base = float(self.dist.rvs(1))
